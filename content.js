...
function createParticle() {
    ...
    particle.direction.x = Math.random() * Math.sign(Math.random() - .5);
    particle.direction.y = Math.sqrt(1 - Math.pow(particle.direction.x, 2));
    particle.scale = Math.random() * 2 + 1;
    particle.speed = .2 / particle.scale;
    return particle;
}
function loop() {
    ...
    for (var i = 0; i < particles.length; i++) {
        var p = particles[i];
        var w = 10 * p.scale;
        ctx.drawImage(particleImage, p.position.x*window.innerWidth/100 - w/2, p.position.y*window.innerHeight/100 - w/2, w, w);
        p.position.x += p.direction.x * p.speed + mouseX/windowHalfX*p.speed;
...
function start_snow(){
    canvas.style.backgroundColor = '#00000030';
    snow_interval = setInterval(loop, 1000 / 40);
}
function stop_snow() {
